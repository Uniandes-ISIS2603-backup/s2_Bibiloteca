...
public class ReservaResource {
    ...
    //__________________________________________________________________________
    /**
     * CREA UNA NUEVA RESERVA Crea una nueva reserva con la informacion que se
     * recibe en el cuerpo de la petición y se regresa un objeto identico con un
     * id auto-generado por la base de datos.
     *
     * @param pReserva {@link ReservaDTO} - La reserva que se desea guardar.
     * @return JSON {@link ReservaDTO} - La reserva guardada con el atributo id
     * autogenerado.
    ...
    public ReservaDTO createReserva(ReservaDTO pReserva) throws BusinessLogicException {
        LOGGER.log(Level.INFO, "ReservaResource createReserva: input: {0}", pReserva);
        ...
        LOGGER.log(Level.INFO, "ReservaResource createReserva: output: {0}", reservaDTO);
        return reservaDTO;
    }
     ...
     *
     * @return JSONArray {@link ReservaDetailDTO} - Las reservas encontradas en
     ...
     * OBTIENE UNA RESERVA
     *
     * @param reservasId recibe por parametro el id de la reserva que esta
     * buscando
     ...
     *
     * @param reservaId recibe por parametro el id de la reserva que va a
     * actualizar
     * @param reserva
    ...
    public ReservaDTO updateReserva(@PathParam("reservasId") Long reservaId, ReservaDTO reserva) {
        ...
        if (reservaLogic.getReserva(reservaId) == null) {
            throw new WebApplicationException(RUTA + reservaId + NO_EXISTE, 404);
        }
     ...
     * de objetos ReservaEntity a una lista de objetos ReservaDetailDTO (json)
     *
